...
class MIDTPSender:
    ...
    def send_data(self, data: bytes) -> float:
        ...
        print(
            f"[MIDTP] Data size: {len(data)} bytes, Chunk size: {self.chunk_size} bytes"
        )
        ...
        while retry_count < MIDTP_MAX_RETRIES:
            ...
            if response is None:
                ...
                retry_count += 1
                print(
                    f"[MIDTP] ⏱️  Timeout waiting for response (retry {retry_count}/{MIDTP_MAX_RETRIES})"
                )
                continue
            ...
            if response == "ACK":
                ...
                print(
                    f"[MIDTP] 📊 Total packets sent (including retransmissions): {total_packets_sent}"
                )
                break
                ...
                retry_count += 1
                print(
                    f"[MIDTP] ⚠️  NACK received: {len(packets_to_send)} packets need retransmission"
                )
                print(
                    f"[MIDTP] 🔍 Lost packet IDs: {sorted(list(packets_to_send))[:20]}{'...' if len(packets_to_send) > 20 else ''}"
                )
                print(
                    f"[MIDTP] 🔄 Starting retransmission round {retry_count}/{MIDTP_MAX_RETRIES}"
                )
        ...
        if retry_count >= MIDTP_MAX_RETRIES:
            print(
                f"[MIDTP] ⚠️  Warning: Max retries reached, transmission may be incomplete"
            )
        ...
        print(
            f"[MIDTP]   ✓ Retransmission overhead: {((total_packets_sent - total_packets) / total_packets * 100):.1f}%"
        )
    ...
    def _send_packets(self, chunks: list, packet_indices: Set[int]) -> int:
        ...
        for seq_num in sorted(packet_indices):
            ...
            if sent_count % 100 == 0:
                print(
                    f"[MIDTP] 📤 Sending packets... {sent_count}/{len(packet_indices)}"
                )
    ...
    def send_data(self, data: bytes) -> float:
        ...
        for i in range(0, len(data), chunk_size):
            ...
            if total_sent % (10 * 1024 * 1024) == 0 or total_sent == len(data):
                print(
                    f"[{self.protocol_name}] 📤 Sent {total_sent / (1024*1024):.2f} MB / {len(data) / (1024*1024):.2f} MB"
                )
        ...
        async def send_data_on_stream(self, data: bytes):
            ...
            for i in range(0, len(data), chunk_size):
                ...
                if total_sent % (10 * 1024 * 1024) == 0 or end_stream:
                    print(
                        f"[QUIC] 📤 Sent {total_sent / (1024*1024):.2f} MB / {len(data) / (1024*1024):.2f} MB"
                    )
...
class QUICServer:
    ...
    def __init__(self, host: str, port: int):
        if not QUIC_AVAILABLE:
            raise ImportError(
                "aioquic 라이브러리가 설치되지 않았습니다. 'pip install aioquic'를 실행하세요."
            )
...
예시:
  ...
  python3 sender.py --protocol quic --host 127.0.0.1 --port 5003 --file-size 100
    ...
    parser.add_argument(
        "--port", type=int, default=5000, help="포트 번호 (기본값: 5000)"
    )
    ...
    try:
        ...
        if args.protocol == "midtp":
            ...
            print(
                "[TCP-BBR] 주의: BBR 혼잡 제어 알고리즘이 활성화되어 있는지 확인하세요."
            )
            print(
                "[TCP-BBR] Linux 커널 설정: sudo sysctl -w net.ipv4.tcp_congestion_control=bbr"
            )
